...
router.get('/accountByID/:id', async function(req, res, next) {
  ...
  try {
    ...
    if (data) {
      // Phát sự kiện realtime cho tất cả client khi tài khoản được cập nhật
      req.app.get('io').emit('accountDetails', data);
      // Trả về dữ liệu tài khoản cho client
      res.json(data);
    } else {
      res.status(404).json({ message: 'Tài khoản không tìm thấy' });
    }
  } catch (error) {
    console.error('Error fetching account:', error); // Log rõ lỗi
