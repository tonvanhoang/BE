...
router.get('/friendsPosts/:idAccount', async function(req, res, next) {
  try {
    ...
    const posts = await modelsPost.find({
      private: 'Bạn bè', // Chỉ lấy bài viết có quyền riêng tư 'friends'
...
router.get('/postByAccount/:id', async function(req, res, next) {
  try {
    const { id } = req.params;
    const data = await modelsPost.find({ 'idAccount': id ,statusPost:'on',private:'Bạn bè'});
    res.status(200).json(data);
  } catch (error) {
...
// riêng tư
router.get('/postByAccountPrivate/:id', async function(req, res, next) {
  try {
    const { id } = req.params;
    const data = await modelsPost.find({ 'idAccount': id ,statusPost:'on',private:'Chỉ mình tôi'});
    res.status(200).json(data);
  } catch (error) {
    res.status(500).json({ message: 'Đã xảy ra lỗi khi lấy bài viết theo tài khoản', error });
  }
...
router.put('/editPrivate/:id', async function(req, res, next) {
  try {
    const { id } = req.params
  const { private } = req.body;
  const data = await modelsPost.findByIdAndUpdate(id, { private }, { new: true })
